...
ExtraView.prototype.confirmSpend = function (userId, mapIndex, type, price) {
	...
	var confirmStr = '';
	if (type === "buy") {
		confirmStr = "你可以买下这个矿场，需要花费" + price + "比特，确定买么？";
	}

	if (type === "upgrade") {
		confirmStr = "只需要" + price + "比特就可以升级您的矿机，要升级么？";
	}
...
var extraView = {
	...
	confirm: function (user, type, price) {
		...
		var tips = '你确定花' + price + '比特' + (type == 'buy' ? '购买这片矿场' : '在这里加矿机') + '吗？';//加选择
	...
	gameOver: function (user) {
		console.log('extra.gameOver', user);
	}
};
